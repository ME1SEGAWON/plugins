...
`.trim(), wm, 'Pemilik Bot', '.owner', m)
    ...
    //if (db.data.settings[botNumber].autobio) {
	   // let setting = global.db.data.settings[botNumber]
		...
		await this.setBio(`${conn.user.name} | Runtime : ${runtime(uptime)}`)
...
//	}
  ...
  /*  // update status
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setBio(`Aktif selama ${uptimex} │ Mode: ${global.opts['self'] ? 'Private' : 'Publik'} │ total user : ${totaluser} │ ${global.wm}`)
        setting.status = new Date() * 1
    }
    */
    // backup db
